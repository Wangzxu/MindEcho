...
> 本文档讲述 MindEcho 管理后台中，一份知识文档从用户上传到完成向量嵌入（ChromaDB）的完整逻辑链路。
> 覆盖：前端上传 → 后端接口 → 格式转换 → 对象存储 → 结构化切片 → 批量 Embedding → 向量库写入 → 状态回写。
>
> **架构要点（异步化）**：上传接口只做「校验/去重/落库/暂存/入队」，立即返回 `{import_id}`；
> 真正的转换、切片、向量化在**后台 worker** 中异步执行，前端通过状态接口轮询进度。
...
│        ▼                                                                             │
│  立即返回 {import_id, status:"pending"}  ←── HTTP 请求在此结束（大文档不再超时）       │
│                                                                                      │
...
│        │        → 构建父文档 (parent_content)                                         │
│        │      RecursiveCharacterTextSplitter (800字符/100重叠)                        │
│        │  ⑪ llm_service.batch_embed()  批量向量化                                     │
│        │      （100条/批 × 并发4 × 指数退避重试3次，每50条回写 processed_chunks 进度）  │
...
- 手动录入科普卡片：`POST /api/admin/knowledge/manual`（同步执行，见 §7）
- 旧数据重处理：`POST /api/admin/knowledge/{id}/reprocess`（异步 worker，见 §8）
...
| 顺序 | 步骤 | 说明 |
|---|---|---|
| 1 | `init_mysql()` | 建表（含 `knowledge_imports`）+ **列迁移**：为已存在的表补齐 `processed_chunks` 列 |
| 2 | `vector_db.init_db()` | 初始化 ChromaDB 持久化客户端，预创建 `psychology_kb` 与 `safety_warnings_kb` 两个集合（cosine 度量） |
| 3 | `llm_service.init_service()` | 加载硅基流动 Embedding/LLM 客户端 |
...
- **异步轮询**（改造后）：
  1. 上传接口立即返回 `{import_id}` → 进入 `startPolling(task)`；
  2. 每 1.5s 轮询 `GET /api/admin/knowledge/{import_id}/status`；
  3. 根据 `status` 更新任务卡片：`processing` 时展示**实时进度条**（`processed_chunks / chunk_count`，对应"🧮 向量化"阶段），`success` → 步骤 4 完成，`failed` → 展示后端 `error_message`；
  4. 组件卸载时清理全部轮询定时器（`onUnmounted`）。
- **失败展示**：任务卡片显示 `❌ 失败` + 后端返回的错误详情。
...
整个路由挂在 `admin_bp`（prefix `/api/admin`，依赖 `get_current_admin` 鉴权）。**只做 6 件事，不跑任何重逻辑**：

1. **读取字节流**：`content_bytes = await file.read()`。
2. **SHA256 哈希去重**：若 `knowledge_imports.file_hash` 已存在 → 400「该文档已导入，请勿重复上传」。
3. **同名覆盖**：若存在 `file_name` 相同的旧记录（内容不同）→ 视为更新：删除旧版本的 ChromaDB chunks + MySQL 记录，**只留存最新一份**；被覆盖的旧 import_id 通过响应 `replaced_import_id` 返回，供前端提示。
4. **写 MySQL 任务记录**（`status="pending"`，`processed_chunks=0`）：
   ```python
   task = KnowledgeImport(
       file_name=file.filename, file_hash=file_hash,
       minio_bucket=storage_service.bucket_name or "local-upload",
       minio_object_name=f"uploads/{file_hash}_{file.filename}",
       file_size=file_size,
       status="pending", chunk_count=0, processed_chunks=0)
   ```
...
**立即返回** `{import_id, chunk_count: 0, status: "pending", replaced_import_id: 旧id | null}`。

> **为什么不会阻塞 event loop**：`BackgroundTasks` 对同步 callable 在线程池中执行（Starlette 行为），
> 因此后台的 `ingest_document` 不会卡住聊天 SSE 等其他异步请求。
...
在独立线程 + **独立 DB 会话**（`SessionLocal()`，不复用请求级 session）中执行，**全程维护状态机**：
...
def process_knowledge_import(import_id, file_bytes=None, delete_existing=False):
    ...
    try:
        ...
        db.commit()
        if file_bytes is None:        # upload 未传内存字节（如 reprocess）→ 从 MinIO 读取
...
**状态查询接口**：`GET /api/admin/knowledge/{import_id}/status` → 返回 `task.to_dict()`
（含 `status / chunk_count / processed_chunks / error_message / file_name`），供前端轮询。
...
调用 `convert_to_markdown(file_bytes, filename)`（`backend/app/services/converter.py`），按扩展名分发：
...
**失败熔断**：若转换结果为空 → `raise ValueError("文档转换后内容为空")`。

### 6.2 MinIO 对象存储（物理备份）

`storage_service.upload_file()` 上传两份，**失败不阻断主流程**（只记 warning）：
...
### 6.3 结构化切片 `split_markdown_into_chunks`

`backend/app/services/rag.py`（L56 起），**两层切片架构**：

**第一层：标题边界切片**
- `MarkdownHeaderTextSplitter(headers_to_split_on=[("#","h1"),("##","h2"),("###","h3")], strip_headers=False)`。
- 按相同 `(h1, h2)` 归组，拼接为该小节的**父文档** `parent_content`（含标题行）。

**第二层：字符级二级切分**
- 将父文档按 `### ` 拆成 H3 段，每段再用 `RecursiveCharacterTextSplitter(chunk_size=800, chunk_overlap=100)` 切分，保证 chunk 边界不跨 H3 标题。

**产出**：每个 chunk 携带完整结构化元数据：
...
- 每批最多 100 条文本；最多 **4 个并发批次**（`ThreadPoolExecutor`）；
- 单批失败按 **2^n 秒指数退避** 重试 3 次，最终仍失败则该批降级为全零向量（不中断主流程）；
- 结果与输入 `texts` 一一对应。
- 正常模式：`OpenAIEmbeddings.embed_documents`（批量一次请求多条，模型默认 `BAAI/bge-large-zh-v1.5`）；
...
5. **metadata 只有 3 个字段**：
   ...
   ```
   ⚠️ **没有 `parent_content` / `section_id` / `h1/h2/h3`**——这是链路调试中"找不到父文档"的两大来源之一。当前链路调试已在 Step 4 对此兜底：缺父文档时自动把该组子文档拼接（`source: "children_concat"`）。

---

## 8. 并行路径：旧数据重处理（reprocess，异步）
...
3. 用途：把 §7 之前入库、缺 `parent_content` 的旧数据迁移到新管线（补全父文档结构）。
...
- 覆盖后库中只保留最新一份，不保留版本历史（与"文件名 = 文档唯一标识"的心智一致）。
...
2. 物理删除 MySQL 记录；
3. 返回删除结果。前端列表"🗑️ 删除"按钮带 `confirm` 二次确认。

### 9.5.3 Chunks 可视化（含父文档）
...
前端"📋 Chunks"按钮打开**可视化弹窗**：展示文档基本信息（文件名、chunk 总数）、每个 chunk 的章节路径、子 chunk 内容，以及可折叠的**父文档全文**；无父文档元数据的旧数据/手动录入显示"⚠️ 无父文档"提示。
...
| 路由 | `backend/app/routes/admin.py` | `/knowledge/upload`（异步入口+同名覆盖）、`process_knowledge_import`（worker）、`/knowledge/{id}/status`、`/knowledge/manual`、`/knowledge/{id}/reprocess`、**`DELETE /knowledge/{id}`**、`/knowledge/{id}/chunks`（含父文档）、`/knowledge/trace` |
